    ...
    class Voronoi
    {
        public static List<VoronoiEdge> VoronoiEdges(List<DelaunayEdge> delaunayEdges)
        {
            ...
            for (int i = 0; i < delaunayEdges.Count; i++)
            {
                ...
                for (int j = 0; j < Collections.allRooms[delaunayEdges[i].start].adjoinTriangles.Count; j++)
                {
                    for (int k = 0; k < Collections.allRooms[delaunayEdges[i].end].adjoinTriangles.Count; k++)
                    {
                        if (Collections.allRooms[delaunayEdges[i].start].adjoinTriangles[j] == Collections.allRooms[delaunayEdges[i].end].adjoinTriangles[k])
                        {
                            neighbours.Add(Collections.allRooms[delaunayEdges[i].start].adjoinTriangles[j]);
                        }
                    }
                }
